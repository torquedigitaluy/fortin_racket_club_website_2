...
export default async function Servicios() {
  ...
  return (
    <section id="servicios" className="bg-brand-dark py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {servicios.map((servicio) => (
            <article
              key={servicio.title}
              className="group flex flex-col overflow-hidden rounded-[15px] bg-brand shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl"
            >
              {/* Imagen */}
              ...
              <div className="flex flex-1 flex-col p-6">
                ...
                <p className="mt-3 flex-1 font-mulish text-sm text-white/70">
                  {servicio.text}
                </p>
                <Link
                  href={servicio.href}
                  className="mt-5 inline-flex items-center gap-1.5 font-mulish text-sm font-semibold text-white transition-colors hover:text-lime"
                >
                  Saber más
